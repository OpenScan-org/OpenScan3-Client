...
function buildDeviceWebSocketUrl(baseUrl: string): string {
  try {
    const url = new URL(baseUrl)
    url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
    const strippedPath = url.pathname.replace(/\/+$|(?<=^)\//g, '').replace(/\/+/g, '/').trim()
    const prefix = strippedPath.length ? `/${strippedPath}` : ''
    let nextPath = `${prefix}/ws/device`.replace(/\/+/g, '/').replace(/\/$/, '')
    ...
    console.error('Invalid base URL for device websocket', error)
...
export const useDeviceStore = defineStore('device', {
  ...
  actions: {
    ...
    async connect() {
      ...
      const wsUrl = buildDeviceWebSocketUrl(apiConfigStore.baseURL)
